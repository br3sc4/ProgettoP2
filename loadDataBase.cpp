...
if (dBMonopattini.is_open()){
    ...
    string targa, posizione;
    double capBatteria, capAttuale;
    while(dBMonopattini >> citta >> targa >> posizione >> km >> potenza >> numMotori >> capBatteria >> capAttuale >> caricaSupp >> inCarica >> colonninaAtt >> numPost >> ingombro){
        model.addVehicle(citta, new Monopattino(targa, posizione, km, potenza, numMotori, capBatteria, capAttuale, static_cast<Monopattino::VelocitaRicarica>(caricaSupp), inCarica, static_cast<Monopattino::Colonnina>(colonninaAtt), numPost, ingombro));
    }
}else std::cout << "non riesco ad aprire il file dBMonopattini\n";
dBMonopattini.close();
...
if (dBMotoElettriche.is_open()){
    ...
    string targa, posizione;
    double capBatteria, capAttuale;
    while(dBMotoElettriche >> citta >> targa >> posizione >> km >> potenza >> numMotori >> capBatteria >> capAttuale >> caricaSupp >> inCarica >> colonninaAtt >> numPost >> ingombro){
        model.addVehicle(citta, new MotoElettrica(targa, posizione, km, potenza, numMotori, capBatteria, capAttuale, static_cast<Monopattino::VelocitaRicarica>(caricaSupp), inCarica, static_cast<Monopattino::Colonnina>(colonninaAtt), numPost, ingombro));
    }
}else std::cout << "non riesco ad aprire il file dBMotoElettriche\n";
...
if (dBMoto.is_open()){
    ...
    string targa, posizione;
    double capSerba, litSerba;
    while(dBMoto >> citta >> targa >> posizione >> km >> capSerba >> litSerba >> carburante >> cilindrata >> emissioni >> numPost >> ingombro){
        model.addVehicle(citta, new Moto(targa, posizione, km, capSerba, litSerba, static_cast<Automobile::Carburante>(carburante), cilindrata, emissioni, numPost, ingombro));
    }
}else std::cout << "non riesco ad aprire il file dBMoto\n";
dBMoto.close();
